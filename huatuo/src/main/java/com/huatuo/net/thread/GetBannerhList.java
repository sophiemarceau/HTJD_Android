...
public class GetBannerhList implements Runnable {
	...
	public GetBannerhList(Context mContext, Handler mHandler, HashMap<String, String> inJsonObject) {

		this.mContext = mContext;
		this.mHandler = mHandler;
		this.inJsonObject = inJsonObject;
		this.bannerList = new ArrayList<JSONObject>();
	}
	...
	public void run() {
		try {
			...
			CommonUtil.logE("获取banner列表的提交参数：------------->" + inJsonObject);

			HttpAgent httpAgent = new HttpAgent("cms/getBannerList",
	...
	private void initRsultData(ActionResponse actionRespons) {
		...
		if(null != bodyJsonObject){
			JsonUtil.jsonArray2List(bodyJsonObject.optJSONArray("bannerList"), bannerList);
		}
	}
	public ArrayList<JSONObject> getBannerList() {
		return bannerList;
	}
