...
/** 获取订单列表 */
public class CancelCollect implements Runnable {
	...
	public CancelCollect(Context mContext, Handler mHandler, HashMap<String, String> map) {
	...
	public void run() {
		try {
			...
			CommonUtil.log("获取取消收藏上传参数：map："+map);
			HttpAgent httpAgent = new HttpAgent("collect/cancelCollect",
					map, mContext);
