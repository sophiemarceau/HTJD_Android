...
public class GetStoreList implements Runnable {
	...
	public GetStoreList(Context mContext, Handler mHandler, HashMap<String, String> inJsonObject) {

		this.mContext = mContext;
		this.mHandler = mHandler;
		this.inJsonObject = inJsonObject;
		this.shopList = new ArrayList<JSONObject>();
	}
	...
	public void run() {
		try {
			...
			CommonUtil.logE("获取搜索店铺列表的提交参数：------------->" + inJsonObject);

			HttpAgent httpAgent = new HttpAgent("search/searchStoreList", inJsonObject, mContext);
	...
	private void initRsultData(ActionResponse actionRespons) {
		...
		shopList = JsonUtil.jsonArray2List(bodyJsonObject.optJSONArray("storeList"));
	}

	public ArrayList<JSONObject> getShopList() {
		return shopList;
	}
