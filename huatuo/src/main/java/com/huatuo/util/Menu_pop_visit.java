...
public class Menu_pop_visit {
	...
	private View mView;
	private static Menu_pop_visit instance;
	private HandleSortType_Interface mhHandleSortType_Interface;
	private int[] sortType_arr_ID = new int[] {R.string.near_fromMine, R.string.appraise_high, R.string.order_more };
	private String[] sortType_arr = new String[3];
	private MyAdapter adapter;

	public static Menu_pop_visit getInstance() {
		if (instance == null) {
			synchronized (Menu_pop_visit.class) {
				if (instance == null) {
					instance = new Menu_pop_visit();
				}
			}
		}
	...
	private void addSortType() {
		for (int i = 0; i < sortType_arr_ID.length; i++) {
			sortType_arr[i] = mContext.getResources().getString(
					sortType_arr_ID[i]);
		}
	}
	 ...
	 */
	public void showPupupWindow(Activity context, View view,
			final HandleSortType_Interface handleSortType_Interface, int type) {
		mContext = context;
		mView = view;
	...
	public void initPopuWindow(View view) {
		...
		mPopupWindow = new PopupWindow(view, CommonUtil.dip2px(mContext, 110),
	...
	class MyItemOnClickListener implements OnItemClickListener {
		...
		public void onItemClick(AdapterView<?> parent, View view, int position,
			...
			if (null != mhHandleSortType_Interface) {
				mhHandleSortType_Interface.callBack(position,
						sortType_arr[position]);
			}
