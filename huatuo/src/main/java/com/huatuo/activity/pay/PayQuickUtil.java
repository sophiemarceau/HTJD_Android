 ...
 * 
 */
public class PayQuickUtil {
	private Activity mContext;
	private static PayQuickUtil instance;
	...
	public static PayQuickUtil getInstance() {
		if (instance == null) {
			synchronized (PayQuickUtil.class) {
				if (instance == null) {
					instance = new PayQuickUtil();
				}
			}
		}
	...
	public void wxPay(Activity activity, JSONObject tenpayData,String orderID) {
		...
		CommonUtil.saveBooleanOfSharedPreferences(mContext.getApplicationContext(), "ISQUICKPAY", true);
		final IWXAPI msgApi = WXAPIFactory.createWXAPI(mContext, null);
		msgApi.registerApp(PayUtil.WX_APP_ID);
		req = new PayReq();
...
//		Log.e("Pay_Activity", "tenpayData-------------->" + tenpayData);
	}
