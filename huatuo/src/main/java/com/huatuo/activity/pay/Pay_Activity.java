 ...
 * 
 */
public class Pay_Activity extends BaseActivity implements OnClickListener {
	...
	private void getOrderInfo() {
		...
		if (bundle != null) {
			servName = bundle.getString("serviceName");
			orderID = bundle.getString("orderID");
			service_price = bundle.getString("payment");
	...
	private HashMap<String, String> initParams() {
		...
		// orderID 订单ID ANS 32 M 订单ID
		// orderType 订单类型 ANS 1 C 0 : 默认服务订单
		// 1：充值
		// payType 支付方式 ANS 32 M 0：线下支付
		// 1：在线支付
		// isAccount 余额支付 ANS 16 C 支付方式payType=1选填
		// 与支付渠道编号至少填一个
		// 1：使用余额（不使用余额不传）
		...
		inJsonMap.put("orderType", orderType);
		...
		builder.setPositiveButton(getString(R.string.common_confirm),
				new DialogInterface.OnClickListener() {
					public void onClick(DialogInterface dialog, int which) {
						dialog.dismiss();
						// 设置你的操作事项
						Intent intent = new Intent(Pay_Activity.this,
								HomeActivity.class);
