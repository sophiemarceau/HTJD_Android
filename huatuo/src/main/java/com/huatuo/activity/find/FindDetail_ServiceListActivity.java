...
public class FindDetail_ServiceListActivity extends BaseActivity implements
		OnClickListener {
	...
	private RelativeLayout rl_find_icon_bg;
	private RelativeLayout rl_find_icon;
	private ImageView iv_top;
	private TextView tv_name;
	private LinearLayout ll_back, ll_share, ll_collect;
	private CustomGridView gv_find_list_service;
	// 获取项目列表
	private GetFindDetail_List getFindDetail_ServieList;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		mContext = this;
		setContentView(R.layout.activity_find_detail_service_list);
		initHandler();
		receiveExtras();
		findViewById();
		setOnClickListener();
	...
	private void findViewById() {
		...
		gv_find_list_service = (CustomGridView) findViewById(R.id.gv_find_list_service);
		adapter_service = new Service_girview_Adapter(this);
		gv_find_list_service.setAdapter(adapter_service);
...
//								 Toast_Util.showToastOnlyOne(mContext, mContext.getResources().getString(R.string.load_no_more_data));
	...
	private void getServiceList() {
		...
		getFindDetail_ServieList = new GetFindDetail_List(mContext,
				serviceList_Handler, inJsonObject);
		Thread thread = new Thread(getFindDetail_ServieList);
		thread.start();
	}
	...
	class ServiceList_Handler extends Handler {
		...
		public void handleMessage(Message msg) {
			...
			case MsgId.DOWN_DATA_S:
				...
				// 获取返回数据的页数
				JSONObject jsonObject = getFindDetail_ServieList.getOutJson();
				servicelist = getFindDetail_ServieList.getServiceList();
				if (null != jsonObject) {
...
//		itemType	类型	ANS	11	M	0 门店 1 服务 2 技师 3 发现
	...
	public void onClick(View v) {
		...
		case R.id.iv_share:// 分享
			UmengShare.getInstance().initShareParams(FindDetail_ServiceListActivity.this, image, title,
