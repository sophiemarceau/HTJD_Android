...
public class FindDetail_StoreListActivity extends BaseActivity implements OnClickListener {
	...
	private CustomListView lv_find_list_store;
	// 获取项目列表
	private GetFindDetail_List getFindDetail_StoreList;
	private Handler storeList_Handler;
	private First_Store_List_Adapter adapter_store;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		mContext = this;
		setContentView(R.layout.activity_find_detail_store_list);
		initHandler();
		receiveExtras();
		findViewById();
		setOnClickListener();
	...
	private void initHandler() {
		storeList_Handler = new StoreList_Handler();
	...
	private void findViewById() {
		...
		lv_find_list_store = (CustomListView) findViewById(R.id.lv_find_list_store);
		adapter_store = new First_Store_List_Adapter(this);
		lv_find_list_store.setAdapter(adapter_store);
	...
	private void scrollListener() {
		scrollView1.setScrollViewListener(new com.huatuo.custom_widget.ScrollViewListener() {
			...
			public void onScrollChanged(ObservableScrollView scrollView, int x, int y, int oldx, int oldy) {
				...
				if (0 != y && 0 != oldy) {
					float alpha = (float) 2 * y / (float) rl_find_icon.getHeight();
	...
	private void getStoreList() {
		...
		inJsonObject.put("pageOffset", pageSize + "");
		 ...
		 String lng = CommonUtil.getStringOfSharedPreferences(getApplicationContext(), "DF_LNG",
				 mContext.getResources().getString(R.string.df_lng));
		 String lat = CommonUtil.getStringOfSharedPreferences(getApplicationContext(), "DF_LAT",
				 mContext.getResources().getString(R.string.df_lat));
		inJsonObject.put("longitude", lng);
		inJsonObject.put("latitude", lat);
		
		
		getFindDetail_StoreList = new GetFindDetail_List(mContext, storeList_Handler, inJsonObject);
		Thread thread = new Thread(getFindDetail_StoreList);
		thread.start();
	}

	class StoreList_Handler extends Handler {
		...
		public void handleMessage(Message msg) {
			...
			case MsgId.DOWN_DATA_S:
				...
				// 获取返回数据的页数
				JSONObject jsonObject = getFindDetail_StoreList.getOutJson();
				storelist = getFindDetail_StoreList.getStoreList();
				if (null != jsonObject) {
	...
	public void onClick(View v) {
		...
		case R.id.iv_share:// 分享
			UmengShare.getInstance().initShareParams(FindDetail_StoreListActivity.this, image, title, description, ID, Constants.SHARE_FIND);
			break;
