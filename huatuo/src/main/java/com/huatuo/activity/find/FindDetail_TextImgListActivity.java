...
public class FindDetail_TextImgListActivity extends BaseActivity implements
		OnClickListener {
	...
	private LinearLayout ll_back, ll_collect, ll_share;
	...
	private RequestRunnable getList;
	private Handler txtImg_Handler;
	// 收藏、分享
	...
	private void initHandler() {
		txtImg_Handler = new TextImg_Handler();
	...
	private void getList() {
		...
		getList = RequestData.getInstance().request(mContext, txtImg_Handler,
				RequestUrl.GET_DISCOVER_ITEM, inJsonObject);
	}

	class TextImg_Handler extends Handler {
		...
		public void handleMessage(Message msg) {
			...
			case MsgId.DOWN_DATA_S:
				...
				// 获取返回数据的页数
				JSONObject jsonObject = getList.getOutJson();
				if (jsonObject != null) {
	...
	private void handleData(JSONObject jsonObject) {
		...
		FindImgTxtContentBean findImgTxtContentBean = JSON.parseObject(
				jsonObject.toString(), FindImgTxtContentBean.class);
		publishDate = findImgTxtContentBean.publishDate;
		image = findImgTxtContentBean.image;
		title = findImgTxtContentBean.title;
		description = findImgTxtContentBean.description;
		textImg = findImgTxtContentBean.content;
		isFavorite = findImgTxtContentBean.isFavorite;// 是否收藏
		// 收藏类型 0：否，1：是，
		...
		CommonUtil.log("description:" + description + "-----");
		if (!TextUtils.isEmpty(description)
				&& !TextUtils.isEmpty(StringUtil.replaceBlank(description))) {
			CommonUtil.log("description:不为空:" + description.trim().length());
			tv_find_info.setVisibility(View.VISIBLE);
			tv_find_info.setText(description.trim());
		} else {
		...
		wv_find_list_textImg.loadDataWithBaseURL(null, textImg, "text/html",
				"utf-8", null);
	}
	...
	public void onClick(View v) {
		...
		case R.id.iv_share:// 分享
			UmengShare.getInstance().initShareParams(
					FindDetail_TextImgListActivity.this, image, title,
