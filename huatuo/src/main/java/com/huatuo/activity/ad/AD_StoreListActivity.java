...
public class AD_StoreListActivity extends BaseActivity implements
		OnClickListener {
	...
	private ListView lv_ad_list_store;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		mContext = this;
		setContentView(R.layout.activity_ad_storelist);
		initHandler();
		findViewById();
		setOnClickListener();
	...
	private void findViewById() {
		...
		mRefreshListView = (PullToRefreshListView) findViewById(R.id.lv_ad_list_store);
		lv_ad_list_store = mRefreshListView.getRefreshableView();
		adapter_store = new First_Store_List_Adapter(this);
		lv_ad_list_store.setAdapter(adapter_store);
		// 加载数据失败页面
		...
		public void handleMessage(Message msg) {
			...
			case MsgId.DOWN_DATA_S:
				...
				if (null != jsonObject) {
					...
					CommonUtil.log("广告店铺总共页数：totalPages：" + totalPages);
					CommonUtil.log("广告店铺总条数：totalCounts：" + totalCounts);
				...
				arraylist = getAD_Detail_List.getStoreList();
				if (pageNo == 1) {

					adapter_store.clear();
				}

				adapter_store.add(arraylist);
				break;
